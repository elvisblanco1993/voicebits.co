<x-guest-layout>
    <div class="max-w-3xl mx-auto py-12 lg:py-24 px-4 sm:px-6 lg:px-8 text-center">
        @if ($subscriber->podcast->cover)
            <img src="{{ route('public.podcast.cover', ['url' => $podcast->url]) }}" alt="{{ $subscriber->podcast->name }}" class="w-16 h-16 md:w-24 md:h-24 mx-auto rounded-md aspect-square">
        @endif
        ...
        <div class="mt-6 px-4 py-8 bg-white shadow rounded-lg">
            ...
            <div class="mt-6 max-w-xs mx-auto">
                <div class="sm:hidden">
                    ...
                    <a href="overcast://x-callback-url/add?url={{ route('private.podcast.feed', ['token' => $subscriber->token, ($subscriber->podcast->passkey) ? 'pwd=' . $subscriber->podcast->passkey: '']) }}" class="mt-4 relative block group px-4 py-3 rounded-md font-medium bg-[#FC7E0F]/10 text-[#FC7E0F] hover:bg-[#FC7E0F] hover:text-white transition-all">
                ...
                <button id="feed-to-clipboard" onclick="copyToClipboard('{{route('private.podcast.feed', ['token' => $subscriber->token, ($subscriber->podcast->passkey) ? 'pwd=' . $subscriber->podcast->passkey: ''])}}')"
